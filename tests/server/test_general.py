...
def test_server_is_alive(server_with_cleandata: Server):
    assert server_with_cleandata.is_alive is True


def test_check_if_observations_exist(server_with_cleandata: Server):
    valid_date = datetime.datetime(2022, 4, 6, 0, 3, 0)
    invalid_date = datetime.datetime(1922, 1, 1, 0, 10, 0)

    datastream = server_with_cleandata.get(Datastream, id=6)
    test_url = server_with_cleandata._get_endpoint_url(datastream) + "/Observations"
...
def test_get_endpoint_url_with_id(server_with_cleandata: Server):
    url = server_with_cleandata._get_endpoint_url(Datastream, id=6)
    assert url == "http://0.0.0.0:8080/FROST-Server/v1.1/Datastreams(6)"


def test_get_endpoint_url_without_id(server_with_cleandata: Server):
    url = server_with_cleandata._get_endpoint_url(Datastream)
    assert url == "http://0.0.0.0:8080/FROST-Server/v1.1/Datastreams"


def test_get_id_from_url(server_with_cleandata: Server):
    valid_url = server_with_cleandata._get_endpoint_url(Datastream, id=6)
    invalid_url = valid_url[:-3]

    id = server_with_cleandata._get_id_from_url(valid_url)
    assert id == 6

    with pytest.raises(ValueError):
        server_with_cleandata._get_id_from_url(invalid_url)


def test_extract_ids_from_navlinks(server_with_cleandata: Server):
    datastream = server_with_cleandata.get(Datastream, id=6)

    data_json_with_ids = server_with_cleandata._extract_ids_from_navlinks(
        datastream.json()
    )
